...
const reg = await fetch(BASE + '/api/auth/register', {
  ...
  body: JSON.stringify({ phone, password: 'test123456', nickname: '600巡检' })
});
...
const edge = spawn(EDGE, [
  ...
  '--user-data-dir=E:\\saixt\\screenshots\\edge-profile-600',
  'about:blank'
], { stdio: 'ignore' });
...
async function shot(name) {
  ...
  if (s?.result?.data) { writeFileSync(`${OUT}\\t600-${name}.png`, Buffer.from(s.result.data, 'base64')); }
}
...
await evalJS(`localStorage.setItem('saixt_token', ${JSON.stringify(token)}); localStorage.setItem('saixt_user', JSON.stringify({ nickname: '600巡检' }));`);
...
for (const [p, n] of pages600) await visit(p, n);
