# -*- coding: utf-8 -*-
"""导出本轮 AI 通用技术新增行(基线=导入前本地备份), 供生产增量迁移。"""
import sqlite3, re, json, collections

BACKUP = r'E:\saixt\server\data\saixt.db.bak-before-tyai'
CUR = r'E:\saixt\server\data\saixt.db'
OUT = r'E:\saixt\server\data\new_questions_tyai.json'
...
print('章节:', dict(collections.Counter(r['chapter'] for r in new_rows)))
# 质检新增行
bad = sum(1 for r in new_rows if not (r['stem'] or '').strip() or len(json.loads(r['options'])) < 2 or not (r['answer'] or '').strip())
print('不合格行:', bad)
con.close()