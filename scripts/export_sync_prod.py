...
l_rows = load(LOCAL)
p_rows = load(PROD)
l_keys = collections.Counter((r['subject'], norm(r['stem'])) for r in l_rows)

rows = []
seen = set()
for r in p_rows:
    key = (r['subject'], norm(r['stem']))
    if l_keys[key] == 0 and key not in seen:
        seen.add(key)
        rows.append(dict(r))
