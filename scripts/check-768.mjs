...
const reg = await fetch(BASE + '/api/auth/register', {
  ...
  body: JSON.stringify({ phone, password: 'test123456', nickname: '平板巡检' })
});
...
// 2) 无头浏览器巡检（768px 平板宽度）
const edge = spawn(EDGE, [
  '--headless=new', '--disable-gpu', '--hide-scrollbars',
  `--remote-debugging-port=${PORT}`,
  '--user-data-dir=E:\\saixt\\screenshots\\edge-profile-768',
  'about:blank'
], { stdio: 'ignore' });
...
async function shot(name) {
  ...
  if (s?.result?.data) { writeFileSync(`${OUT}\\t768-${name}.png`, Buffer.from(s.result.data, 'base64')); }
}
...
await evalJS(`localStorage.setItem('saixt_token', ${JSON.stringify(token)}); localStorage.setItem('saixt_user', JSON.stringify({ nickname: '平板巡检' }));`);
...
for (const [p, n] of tabletPages) await visit(p, n);
